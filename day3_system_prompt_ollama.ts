...
const UserSchema = z.object({
  ...
  role: z.preprocess((val) => {
    ...
    if (s === "Moderator") return "Admin"; // Map common hallucinations
...
async function runArchitectSession() {
  ...
  const response = ai.chat.completions.create({
    model: "llama3.2", // Use "llama3.2" for Ollama
    messages: [
      {
        role: "system",
        content: `
     ...
     // @ts-ignore - Add this for Ollama memory optimization
    options: {
      num_ctx: 1024, // Reduces RAM usage significantly
      low_vram: true
    },
